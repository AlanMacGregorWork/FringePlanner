...
struct FringeAPICodableTests {
    ...
    func testFringeDisabled() throws {
        try testEncodeDecodeEquates(value: FringeDisabled(otherServices: nil, audio: nil, captioningDates: nil, signedDates: nil, audioDates: nil))
        try testEncodeDecodeEquates(value: FringeDisabled(otherServices: true, audio: false, captioningDates: [], signedDates: [], audioDates: []))
        try testEncodeDecodeEquates(value: FringeDisabled(otherServices: false, audio: true, captioningDates: ["item1", "item2"], signedDates: ["valueA", "valueB"], audioDates: ["itemEE", "itemCC"]))
    }
    
    @Test("FringePerformanceSpace")
    func testFringePerformanceSpace() throws {
        try testEncodeDecodeEquates(value: FringePerformanceSpace(name: "Item", ageLimited: nil, wheelchairAccess: false))
        try testEncodeDecodeEquates(value: FringePerformanceSpace(name: "New", ageLimited: true, wheelchairAccess: nil))
        try testEncodeDecodeEquates(value: FringePerformanceSpace(name: nil, ageLimited: false, wheelchairAccess: true))
    }
    ...
    func testFringePerformance() throws {
        ...
        try testEncodeDecodeEquates(value: FringePerformance(title: nil, type: .inPerson, isAtFixedTime: true, priceType: .paid, price: 12.50, concession: nil, priceString: "£12.50", start: testDate, end: testDate.addingTimeInterval(3600), durationMinutes: 60, eventCode: "code1"))
        try testEncodeDecodeEquates(value: FringePerformance(title: "Comedy Show", type: .onlineLive, isAtFixedTime: false, priceType: .payWhatYouCan, price: 5.00, concession: 3.50, priceString: "£5.00 (£3.50)", start: testDate, end: testDate.addingTimeInterval(7200), durationMinutes: 120, eventCode: "code2"))
    }
    ...
    func testFringeEvent() throws {
        ...
        try testEncodeDecodeEquates(value: FringeEvent(title: "Test Show", subTitle: nil, artist: nil, country: nil, descriptionTeaser: nil, code: "TEST1", ageCategory: nil, description: "A test show", festival: "Edinburgh Festival Fringe", festivalId: "fringe", genre: "Comedy", genreTags: nil, performances: [], performanceSpace: FringePerformanceSpace(name: "Main Hall", ageLimited: nil, wheelchairAccess: nil), status: .active, url: URL(string: "https://tickets.edfringe.com/TEST1")!, venue: FringeVenue(code: "VENUE1", description: nil, name: "Test Venue", address: nil, position: .init(lat: 55.9533, lon: -3.1883), postCode: "EH1 1QS", webAddress: nil, phone: nil, email: nil, disabledDescription: nil), website: URL(string: "https://example.com")!, disabled: nil, images: [:], warnings: nil, updated: testDate, year: 2024))
        // Full event with all optional fields
        try testEncodeDecodeEquates(value: FringeEvent(title: "Comedy Spectacular", subTitle: "The Comedy Crew", artist: "John Smith", country: "Scotland", descriptionTeaser: "A hilarious evening", code: "TEST2", ageCategory: "16+", description: "Full show description", festival: "Edinburgh Festival Fringe", festivalId: "fringe", genre: "Comedy", genreTags: "Stand-up, Improv", performances: [], performanceSpace: FringePerformanceSpace(name: "Main Stage", ageLimited: false, wheelchairAccess: true), status: .active, url: URL(string: "https://tickets.edfringe.com/TEST2")!, venue: FringeVenue(code: "VENUE2", description: "A lovely venue", name: "Edinburgh Playhouse", address: "18-22 Greenside Place", position: .init(lat: 55.9571, lon: -3.1856), postCode: "EH1 3AA", webAddress: URL(string: "https://example.com"), phone: "+44123456789", email: "venue@example.com", disabledDescription: "Fully accessible"), website: URL(string: "https://comedyshow.com")!, disabled: FringeDisabled(otherServices: true, audio: true, captioningDates: ["2024-08-10"], signedDates: ["2024-08-11"], audioDates: ["2024-08-10"]), images: ["main": FringeImage(hash: "abc123", orientation: .landscape, type: .hero, versions: ["original": .init(type: .original, width: 1920, height: 1080, mime: "image/jpeg", url: URL(string: "https://example.com/image.jpg")!)])], warnings: "Strong language", updated: testDate, year: 2024))
        // Full event with performances
        // Note: Decode DB disabled as that would require knowledge to be passed in the user info (see `FringePerformance`)
        try testEncodeDecodeEquates(decodeDB: false, value: FringeEvent(title: "Comedy Spectacular", subTitle: "SomeNote", artist: "John Smith", country: "Scotland", descriptionTeaser: "A hilarious evening", code: "TEST2", ageCategory: "16+", description: "Full show description", festival: "Edinburgh Festival Fringe", festivalId: "fringe", genre: "Comedy", genreTags: "Stand-up, Improv", performances: [FringePerformance(title: "Evening Show", type: .inPerson, isAtFixedTime: true, priceType: .paid, price: 15.00, concession: 12.00, priceString: "£15.00 (£12.00)", start: testDate, end: testDate.addingTimeInterval(3600), durationMinutes: 60, eventCode: "TEST2")], performanceSpace: FringePerformanceSpace(name: "Main Stage", ageLimited: true, wheelchairAccess: false), status: .active, url: URL(string: "https://tickets.edfringe.com/TEST2")!, venue: FringeVenue(code: "VENUE2", description: "A lovely venue", name: "Edinburgh Playhouse", address: "18-22 Greenside Place", position: .init(lat: 55.9571, lon: -3.1856), postCode: "EH1 3AA", webAddress: URL(string: "https://example.com"), phone: "+44123456789", email: "venue@example.com", disabledDescription: "Fully accessible"), website: URL(string: "https://comedyshow.com")!, disabled: FringeDisabled(otherServices: true, audio: true, captioningDates: ["2024-08-10"], signedDates: ["2024-08-11"], audioDates: []), images: ["main": FringeImage(hash: "abc123", orientation: .landscape, type: .hero, versions: ["original": .init(type: .original, width: 1920, height: 1080, mime: "image/jpeg", url: URL(string: "https://example.com/image.jpg")!)])], warnings: "Strong language", updated: testDate, year: 2024))
    }
}
